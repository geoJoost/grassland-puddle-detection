...
gdf_anlb = gpd.read_file("output/ANLB_clip.shp").to_crs('EPSG:32631')
gdf_brp = gpd.read_file("output/BRP_filtered.shp").to_crs('EPSG:32631')

# Randomly select 70,000 (about 10% of original sample) polygons from the BRP data
gdf_brp_sample = gdf_brp.sample(n=70000, random_state=1)
...
This function does the following things:
    ...
    2. Retrieve statistics using zonal_stats from rasterstats
"""
...
df_anlb = retrieve_zonal_statistics(gdf_train, "ANLB")
...
sns.lineplot(x='days_since_jan1', y='mean', data=df_anlb, 
             color = anlb_col, label='$\mu$ of ANLB', linestyle = '--')
...
sns.lineplot(x='days_since_jan1', y='mean', data=df_brp, 
             color = brp_col, label='$\mu$ of BRP', linestyle = '--')
...
extent_lst = [
    ...
    [5.5106949090941608, 53.1475020995522840, 5.5108963654507210, 53.1476136227111553], #4 - Utrecht
    [5.5106949090941608,  53.1475020995522840, 5.5108963654507210, 53.1476136227111553] #5 - Gelderland
    ]
...
for i, extent in enumerate(extent_lst):
    ...
    line = sns.lineplot(x='days_since_jan1', y='mean', data=df_pix, 
                        ...
                        label='Representative pixels in ANLB-parcel' if i < 2 else None)

#sns.lineplot(x='days_since_jan1', y='mean', data=df_pix2, color = pix_col, label='BLANK')
...
ax.set_title("Temporal variations of backscatter in different parcels", fontsize=14)
