# -*- coding: utf-8 -*-
"""
Created on Fri Jun 16 11:25:46 2023
...
def CreateRaster(xx, yy, std, gt, proj, driverName, outFile):
    ...
    std[np.isinf(std)] = -99
    rows, cols = np.shape(std)
    transform = rasterio.transform.from_origin(gt[0], gt[3], gt[1], gt[5])
    kwargs = {
        'driver': driverName,
        'height': rows,
        'width': cols,
    ...
    with rasterio.open(outFile, 'w', **kwargs) as dst:
        dst.write(std, 1)
        dst.update_tags(1, nodata=-99)
        dst.compute_statistics(1)
...
if __name__ == '__main__':  
    ...
    for win_size in win_sizes[:]:   
        in_raster = "input/Shapes/crane_kriging_raster_5.tif" #Path to input raster
        win = win_size
        meter = str(win/4)
        ...
        w = Parallel(n_jobs = cpu_count(), verbose=0)(delayed(p_me)(Z[k]) for k in list(range(len(Z))))
